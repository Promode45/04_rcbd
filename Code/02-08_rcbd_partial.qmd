...
# Model fitting
rcbd_mod <- lm(data = rcbd_dfw,
               yield_kgha ~ rep + nrate_kgha * krate_kgha
               )
...
- For this, we use the **fitted vs. residual plot**.  
- What we want to see: no clear pattern in residuals, random cloud of points.  jga;jagjfj;la
...
rcbd_cld_all
```
Adjust in the above code? Anova as the filter to do pwc. None makes the comparison stricter, and to avoid false positives. 
